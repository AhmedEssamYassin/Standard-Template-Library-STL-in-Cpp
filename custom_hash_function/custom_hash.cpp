...
- In competitive programming (e.g., Codeforces), adversarial test cases can force worst-case O(N) performance due to
hash collisions.
...
Methods:
--------
CustomHash: Implements the custom hashing function.
unordered_map<Key, Value, CustomHash>: Hash map using custom hash.
unordered_set<Key, CustomHash>: Hash set using custom hash.
*/
...
int main()
{
    ...
    unordered_map<ll, ll, CustomHash> umap;
    umap[1] = 100;
    umap[2] = 200;
    umap[3] = 300;
    ...
    unordered_set<ll, CustomHash> uset;
    uset.insert(5);
    uset.insert(10);
    uset.insert(15);
