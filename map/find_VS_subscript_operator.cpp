...
- Using operator[]: If the key does not exist, it creates a new entry with default value (0 for int, empty string for
string, etc.), leading to unwanted memory usage.
...
int main()
{
    ...
    if (myMap["Alice"])
        cout << "Alice exists with value: " << myMap["Alice"] << "\n";
    else
        cout << "Alice does not exist but was inserted with default value: " << myMap["Alice"] << "\n";

    /* Using find() (Better approach) */
    if (myMap.find("Bob") != myMap.end())
        cout << "Bob exists with value: " << myMap["Bob"] << "\n";
    else
        cout << "Bob does not exist, no insertion occurred." << "\n";

    /* Using count() (Another safe approach) */
    if (myMap.count("Charlie"))
        cout << "Charlie exists with value: " << myMap["Charlie"] << "\n";
    else
        cout << "Charlie does not exist, no insertion occurred." << "\n";
