...
int main()
{
	...
	// Swapping pairs
	p1.swap(p2);
	cout << "After swap - Pair 1: (" << p1.first << ", " << p1.second << ")" << endl;

	// Using tie to unpack a pair
	int num;
	string name;
	tie(num, name) = p1;
	cout << "Unpacked from p1: " << num << " - " << name << endl;

	// Tuple Example
	tuple<int, string, double> t1 = make_tuple(1, "Charlie", 75.5);
	cout << "Tuple: (" << get<0>(t1) << ", " << get<1>(t1) << ", " << get<2>(t1) << ")" << endl;

	// Modifying a tuple element
	get<1>(t1) = "David";
	cout << "After modification: " << get<1>(t1) << endl;
	...
	cout << "Unpacked from tuple: " << id << " - " << person << " - " << weight << endl;
